...
function filter(mass) {
    return mass.filter(item=>{
        katalog__price[0].value < item.price && katalog__price[1].value > item.price ? true : false         
        katalog__price[2].value < item.content && katalog__price[3].value > item.content ? true : false    
        ...
        katalogSize.addEventListener("click", (e)=>{
            if (e.target.className == "katalog__Checkbox" ) {
                e.target.value == item.size ? true : false
            }
        })
        if (mass == []) {
